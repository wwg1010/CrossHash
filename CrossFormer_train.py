...
def get_config():
    config = {
        ...
        "crop_size": 224,
        "batch_size": 64,
        # "datasets": "mirflickr",       #38
        # "datasets": "cifar10",
        "datasets":'mirflickr',  #mirflickr  nuswide_21
        # "datasets":'coco',             #80
        "Label_dim" : 38,   # flickr:38  coco:80
        "epoch": 600,
        "test_map": 30,
        "save_path": "save/CrossFormer",
        "device": torch.device("cuda:2"),
        'test_device':torch.device("cuda:2"),
    ...
    for epoch in range(config["epoch"]):
        ...
        for image, label, ind in train_loader:
            ...
            with torch.cuda.amp.autocast():
                ...
                # train_loss += q_loss.item() + 0.2 * loss2.item()
                train_loss += q_loss.item() + config["alpha"] * loss2.item()
                # train_loss += 0.1    # 消融实验：仅采用位置量化损失函数
                # train_loss += loss2.item()  # 消融实验：仅采用对比损失函数
            optimizer.zero_grad()
            rela_optimizer.zero_grad()

            is_second_order = hasattr(optimizer, 'is_second_order') and optimizer.is_second_order
            # loss_scaler(q_loss + 0.1*loss2 , optimizer, clip_grad=None, parameters=model.parameters(), create_graph=is_second_order)
            loss_scaler(q_loss + config["alpha"] * loss2 , optimizer, clip_grad=None, parameters=model.parameters(), create_graph=is_second_order)
...
# /home/wbt/conda_env_with_new_amp/conda_env/anaconda3/bin/python3.9 CMT_train.py --output_dir './' --model cmt_s --batch-size 32 --apex-amp --input-size 224 --weight-decay 0.05 --drop-path 0.1 --epochs 300 --test_freq 100 --test_epoch 260 --warmup-lr 1e-7 --warmup-epochs 20
        ...
        if (epoch + 1) % config['test_map'] == 0 :
            Best_mAP, index_img = validate(config, Best_mAP, test_loader, dataset_loader, model, bit, epoch, num_dataset)
            model.to(config["device"])
